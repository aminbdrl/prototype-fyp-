...
if st.sidebar.button("🔄 Refresh Analysis"):
    ...
    daily = (
        df_tweets.groupby(["date_only", "sentiment"])
        .size()
        .reset_index(name="count")
    )
    ...
    # =====================================
    # 📅 PREDICT TREND SENTIMENT TIMELINE
    # =====================================
    st.subheader("📅 Predict Trend Sentiment Timeline")

    # Calculate dominant sentiment for each future day
    timeline_data = []
    ...
    for sentiment in daily["sentiment"].unique():
        ...
        for i, (date, count) in enumerate(zip(future_dates, y_pred)):
    ...
    # Find dominant sentiment for each day
    dominant_sentiment = (
        timeline_df.groupby("date")
        .apply(lambda x: x.loc[x["predicted_count"].idxmax(), "sentiment"])
        .reset_index(name="dominant_sentiment")
    )
    
    # Get total predicted tweets per day
    total_per_day = (
        timeline_df.groupby("date")["predicted_count"]
        .sum()
        .reset_index(name="total_tweets")
    )
    
    # Merge
    timeline_summary = dominant_sentiment.merge(total_per_day, on="date")
    
    # Display timeline
    for _, row in timeline_summary.iterrows():
        ...
        st.markdown(
            f"""
            <div style="padding: 10px; margin: 5px 0; border-left: 4px solid {color}; background-color: rgba(255,255,255,0.05); border-radius: 5px;">
                <strong>{emoji} {date_str}</strong> - Dominant Sentiment: <span style="color: {color};">{sentiment.upper()}</span> ({total} predicted tweets)
            </div>
            """,
    ...
    with st.expander("ℹ️ About This Dashboard"):
        ...
        **Features:**
        ...
        - Predicts future sentiment trends with timeline visualization
